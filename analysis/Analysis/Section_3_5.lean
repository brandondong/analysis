...
theorem SetTheory.Set.iProd_empty_iff {n:ℕ} {X: Fin n → Set} :
    iProd X = ∅ ↔ ∃ i, X i = ∅ := by
  constructor <;> intro h
  . -- Contrapositive: for all i, X i is not empty, prove there is some element in iProd X
    -- Consider the tuple which for each argument, performs a .choose.
    contrapose! h
    simp
    rw [SetTheory.Set.ext_iff]
    push_neg
    set f := ((fun i ↦ ⟨ (nonempty_def (h i)).choose, by {
      set c := nonempty_def (h i)
